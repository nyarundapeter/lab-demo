  ...
  return (
    <div className="container-page max-w-md py-12">
      ...
      <p className="mt-2 text-ink-500">
        One-time setup — after this, booking a visit just needs a confirmation.
      </p>

      <form onSubmit={handleSubmit} className="mt-8 space-y-5 rounded-card border border-ink-100 bg-white p-6 shadow-sm">
        <div>
          <label className="field-label" htmlFor="name">Full name</label>
          <input id="name" className="field-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Amara Osei" />
        ...
        <div>
          ...
          <input id="phone" className="field-input" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+254 700 000 000" />
          {errors.phone && <p className="mt-1 text-xs text-red-600">{errors.phone[0]}</p>}
          <p className="mt-1 text-xs text-ink-400">Used to find your bookings — no separate account number.</p>
        </div>
        <div>
